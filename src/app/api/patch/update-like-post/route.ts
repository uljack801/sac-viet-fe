...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const index = UpdateConversation.likes.findIndex(
      (like: { user: string }) => like.user.toString() === decoded.id
    );
    
    if (index === -1) {
      UpdateConversation.likes.push({ user: decoded.id });
    } else {
      UpdateConversation.likes.splice(index, 1);
    }
