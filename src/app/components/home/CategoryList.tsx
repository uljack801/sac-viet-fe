...
export const CategoryList = ({listCategory}: {listCategory: CategoryProps | null}) => {
    ...
    return (
        ...
        {listCategory?.data.map((value, idx) => {
            return (
              <div key={idx} className="flex flex-col justify-center items-center text-sm font-medium bg-white m-1 p-4 rounded-xl shadow hover:scale-[1.03] hover:bg-neutral-100 text-center min-h-24" onClick={() => route.push(`/${value.slug}`)}>
